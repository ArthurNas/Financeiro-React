...
const INITIAL_STATE = { id: '', descricao: '', isAporte: false, valorEstimadoMensal: '' };
...
function CadastroTipo() {
  ...
  const [formData, setFormData] = useState(
    editando
      ? {
          ...
          isAporte: state.tipo.isAporte ?? false,
          valorEstimadoMensal: state.tipo.valorEstimadoMensal ?? '',
        }
      : { ...INITIAL_STATE }
  );
  ...
  const handleSubmit = (e) => {
    ...
    const dadosParaEnviar = {
      ...
      isAporte: formData.isAporte,
      valorEstimadoMensal: formData.valorEstimadoMensal === '' || formData.valorEstimadoMensal == null
        ? null
        : Number(formData.valorEstimadoMensal),
    };

    const request = editando
      ? tipoService.atualizar(formData.id, dadosParaEnviar)
      : tipoService.salvar(dadosParaEnviar);
          ...
          <form id="form-tipo" onSubmit={handleSubmit}>
            ...
            <div className="mb-4">
              <InputMoeda
                ...
                name="valorEstimadoMensal"
                value={formData.valorEstimadoMensal}
                onChange={handleChange}
              />
              <p className="mt-1 text-xs text-gray-500">Opcional. Use para definir o teto mensal dessa categoria.</p>
            </div>
